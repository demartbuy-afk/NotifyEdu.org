...
interface TeacherAttendanceHistoryModalProps {
  isOpen: boolean;
  onClose: () => void;
  teacher: Teacher;
}

const TeacherAttendanceHistoryModal: React.FC<TeacherAttendanceHistoryModalProps> = ({ isOpen, onClose, teacher }) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (isOpen && user) {
      const fetchHistory = async () => {
        ...
        setError(null);
        try {
          const historyLogs = await api.getTeacherAttendanceHistory(teacher.teacher_id, user.token);
          setLogs(historyLogs);
        } catch (err) {
  ...
  const renderContent = () => {
    ...
    return <AttendanceLogList logs={logs} emptyMessage="No attendance history found for this teacher." />;
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl">
        <div className="p-6 border-b dark:border-gray-700 flex justify-between items-center">
          <h3 className="text-xl font-semibold leading-6 text-gray-900 dark:text-gray-100">
            Attendance History for {teacher.name}
          </h3>
...
export default TeacherAttendanceHistoryModal;