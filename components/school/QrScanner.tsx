...
const QrScannerModal: React.FC<QrScannerModalProps> = ({ isOpen, onClose, onSuccess }) => {
  const { user } = useAuth();
  const [scanResult, setScanResult] = useState<{ type: 'success' | 'error' | 'loading', message: string } | null>(null);
  const readerId = `qr-reader-school-${Math.random().toString(36).substr(2, 9)}`;
  ...
  const handleQrCode = async (decodedText: string) => {
    try {
      ...
      const { log, entityName } = await api.markAttendanceByQr(user.id, user.token, decodedText);
