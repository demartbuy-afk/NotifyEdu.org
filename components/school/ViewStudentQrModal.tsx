...
interface ViewStudentQrModalProps {
  isOpen: boolean;
  onClose: () => void;
  student: Student;
}

const ViewStudentQrModal: React.FC<ViewStudentQrModalProps> = ({ isOpen, onClose, student }) => {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-sm">
        ...
        <div className="p-6 text-center space-y-4">
            <div ref={qrCodeRef} className="flex justify-center bg-white p-4 rounded-lg">
                <QRCodeSVG value={student.qr_value} size={200} level="H" />
            </div>
            <div className="text-left bg-gray-50 dark:bg-gray-700 p-4 rounded-lg space-y-2 text-gray-800 dark:text-gray-200">
                <p><strong>Name:</strong> {student.name}</p>
                <p><strong>Student ID:</strong> <code className="bg-gray-200 dark:bg-gray-600 px-1 rounded">{student.student_id}</code></p>
...
export default ViewStudentQrModal;